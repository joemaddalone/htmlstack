...
var player = function player(v, file, w, h) {
    this.go = null;
...
player.prototype = {
    ...
    scrubberEvent: function scrubberEvent(el, action) {
    ...
    drawProgress: function drawProgress(control) {
        if (control) {
    ...
    drawVolume: function drawVolume(control) {
        this.clearControl(control);
        this.fillControl(control);
        var xpos = control.width * this.vid.volume;
        this.drawScrubber(control, xpos);
    },
    drawScrubber: function drawScrubber(control, xpos) {
        if (control && xpos) {
    ...
    setScrubber: function setScrubber(e, control, func) {
        if (control.isDrag) {
            var ox = e.pageX;
    ...
    fillControl: function fillControl(control) {
        if (control) {
            control.ctx.fillStyle = control.bg;
            control.ctx.fillRect(0, 0, control.width, control.height);
        }
    },
    drawControl: function drawControl(ident, w, h, color) {
    ...
    clearControl: function clearControl(control) {
        if (control) {
            control.ctx.clearRect(0, 0, control.width, control.height);
            ;
        }
    },
    writeControl: function writeControl(control, txt) {
        this.clearControl(control);
        this.fillControl(control);
