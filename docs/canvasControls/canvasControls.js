...
player.prototype = {
    ...
    setup: function () {
		...
		//this.c.addEventListener('dblclick', function (e) {p.screen()}, false);
        this.c.addEventListener('click', function (e) {p.play()}, false);
        this.go = setInterval(function () {p.drawFrame()}, 30)
		
        this.scrubberEvent(this.vscrub, "time")
        this.scrubberEvent(this.vol, "vol")
        //this.fscr.addEventListener('click', function () {p.screen();}, false);
		this.resizeVideo()
		p.play()
		 /*
    ...
    scrubberEvent: function (el, action) {
        ...
        el.addEventListener('mouseout', function(){this.isDrag = false}, false);
    ...
    play: function () {
        if (this.vid.paused) {
			...
			this.goScrubber = setInterval(function () {p.drawProgress(p.vscrub)},100)
        } else {
            if (this.vid.ended) {
